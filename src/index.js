...
async function createMap(element, layout, handleSelect) {
  ...
  try {
    ...
    function getCoordinates(addressInfo) {
      ...
      if (addressInfo.state === "California" && californiaCoordinates[addressInfo.city]) {
        return californiaCoordinates[addressInfo.city];
      }
      
      // If we recognize the city in Connecticut
      if (addressInfo.state === "Connecticut" && connecticutCoordinates[addressInfo.city]) {
    ...
    const processedData = qMatrix.map(row => {
      ...
      const qElemNumber = row[0].qElemNumber; // Store the element number for selections
      ...
      return {
        ...
        lat: coordinates.lat,
        lng: coordinates.lng,
        qElemNumber: qElemNumber // Add element number
      };
    });
    ...
    const circleScale = d3.scaleSqrt()
      .domain(valueExtent)
      .range([5, 20]);  // Min and max circle radius
  ...
  d3.select(this)
    ...
    svg.append("text")
      ...
      .attr("font-size", "12px")
      .attr("fill", "#666")
      .text("Click on a point to see its value");
